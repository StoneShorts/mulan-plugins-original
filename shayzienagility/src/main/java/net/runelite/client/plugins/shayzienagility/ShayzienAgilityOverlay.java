...
@Slf4j
@Singleton
class ShayzienAgilityOverlay extends OverlayPanel
{
    private final ShayzienAgilityPlugin plugin;
    private final ShayzienAgilityConfig config;

    String timeFormat;
    private String infoStatus = "Starting...";

    @Inject
    private ShayzienAgilityOverlay(final Client client, final ShayzienAgilityPlugin plugin, final ShayzienAgilityConfig config)
    {
        super(plugin);
    ...
    public Dimension render(Graphics2D graphics)
    {
        ...
        {
            panelComponent.setBackgroundColor(new Color(181, 47, 47, 70));; //kleur van de infobox in dit geval crystal
            panelComponent.setPreferredSize(new Dimension(200, 180));
            panelComponent.setBorder(new Rectangle(5, 5, 5, 5));
            panelComponent.getChildren().add(TitleComponent.builder()
                    .text("Shayzien Agility by Mulan")
