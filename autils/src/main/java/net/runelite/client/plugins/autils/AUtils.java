...
*/
	...
	*/
	...
	public void closeBank()
	{
		...
		targetMenu =(new NewMenuEntry("", "", 1, MenuAction.CC_OP.getId(), 11, 786434, false)); //close bank
		...
		delayMouseClick(new Point(0, 0), calc.getRandomIntBetweenRange(10, 100));
	}
	...
	public void depositAll()
	{
		...
		executorService.submit(() ->
		{
			...
			if (isDepositBoxOpen()) {
				targetMenu = new NewMenuEntry("", "", 1, MenuAction.CC_OP.getId(), -1, 12582916, false); //deposit all in bank interface
			}
			else {
				targetMenu = new NewMenuEntry("", "", 1, MenuAction.CC_OP.getId(), -1, 786474, false); //deposit all in bank interface
			}
	...
	public void depositAllOfItem(WidgetItem item)
	{
		...
		targetMenu = new NewMenuEntry("", "", (depositBox) ? 1 : 8, MenuAction.CC_OP.getId(), item.getIndex(),(depositBox) ? 12582914 : 983043, false);
		click(item.getCanvasBounds());
	}
	...
	public void useWallObjectDirect(WallObject targetObject, long sleepDelay, int opcode)
	{
		if(targetObject!=null) {
			...
			targetMenu = new NewMenuEntry("","",targetObject.getId(),opcode,targetObject.getLocalLocation().getX(),targetObject.getLocalLocation().getY(),false);
			setMenuEntry(targetMenu);
	...
	public void useGameObjectDirect(GameObject targetObject, long sleepDelay, int opcode)
	{
		if(targetObject!=null) {
			...
			targetMenu = new NewMenuEntry("","",targetObject.getId(),opcode,targetObject.getSceneMinLocation().getX(),targetObject.getSceneMinLocation().getY(),false);
			setMenuEntry(targetMenu);
			if(targetObject.getConvexHull()!=null){
				delayMouseClick(getRandomNullPoint(),sleepDelay);
			} else {
	...
	public void depositOneOfItem(WidgetItem item)
	{
		...
		targetMenu = new NewMenuEntry("", "", (client.getVarbitValue(6590) == 0) ? 2 : 3, MenuAction.CC_OP.getId(), item.getIndex(),(depositBox) ? 12582914 : 983043, false);
	...
	public void withdrawAllItem(Widget bankItemWidget)
	{
		executorService.submit(() ->
		{
			targetMenu = new NewMenuEntry("Withdraw-All", "", 7, MenuAction.CC_OP.getId(), bankItemWidget.getIndex(), 786445, false);
			clickRandomPointCenter(-200, 200);
		});
	...
	public void withdrawItem(Widget bankItemWidget)
	{
		executorService.submit(() ->
		{
			targetMenu = new NewMenuEntry("", "", (client.getVarbitValue(6590) == 0) ? 1 : 2, MenuAction.CC_OP.getId(), bankItemWidget.getIndex(), 786445, false);
			setMenuEntry(targetMenu);
			clickRandomPointCenter(-200, 200);
		});
